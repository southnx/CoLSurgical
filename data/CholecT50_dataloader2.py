...
def make_video_clip(root_dir, video_folders: list, label_files: list = None, num_frames=8):
    ...
    for i in range(len(video_folders)):
        ...
        frame_names = [join(video_folders[i], frames[j])
                       for j in range(len(frames))]
        num_clips = np.ceil(len(frame_names) / num_frames)
        if label_files:
            labels = json.load(open(join(root_dir, 'labels', label_files[i]), 'rb'))[
                "annotations"]
...
class CholecT50(Dataset):
    def __init__(self, root_dir, mode: str, train_split: list, val_split: list, test_split: list):
        ...
        self.mode = mode
        if self.mode == 'train' or self.mode == 'train1':
        ...
        self.real_data = make_video_clip(
            self.root_dir, self.real_video_folders, self.real_label_files)
    ...
    def __getitem__(self, idx):
        ...
        for label in labels:
            ...
            verb_labels.append(torch.from_numpy(verb_label))
        # print("clip_imgs: ", len(clip_imgs))  # 8
        # print("clip_labels: ", len(clip_labels))
        sample = {'trip labels': torch.stack(trip_labels),
                  'verb labels': torch.stack(verb_labels),
...
def collate_fn_vid(batch):
    ...
    batch_trip_labels, batch_verb_labels, batch_frame_names = [], [], []
    for i in range(size):
        batch_trip_labels.append(batch[i]['trip labels'])
        batch_verb_labels.append(batch[i]['verb labels'])
        batch_frame_names.append(batch[i]['frame names'])

    return torch.stack(batch_trip_labels), torch.stack(batch_verb_labels), batch_frame_names
